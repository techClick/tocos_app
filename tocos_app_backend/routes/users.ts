...
router.get('/users/:id', async (req: Express.Request<{ id: string }>, res: Express.Response) => {
  try {
    ...
    if (!allUsersLength) return res.status(400).json((networkResponse('error', 'Please create new user first')))
    if (Number(queryId) > allUsersLength) {
      return res.status(400).json((networkResponse('error', `No such user. Last user id is ${allUsersLength}.`)))
    }
    result = await client.query(`Select tocos from Users WHERE id=${queryId}`)
